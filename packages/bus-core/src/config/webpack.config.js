...
module.exports = options => {
	...
	if (hasBabelRc) {
		...
		mainBabelOptions
			.presets
			.push(require(resolve('../../babel.js')))
	}

	return {
		// Webpack v4 add a mode configuration option tells webpack to use its built-in
		// optimizations accordingly. @see https://webpack.js.org/concepts/mode/
		mode: options.env === 'development'
			? 'development'
			: 'production',
		// Webpack can target multiple environments such as `node`, `browser`, and even
		// `electron`. Since Bus is focused on Node, we set the default target
		// accordingly.
		target: 'node',
		...
		// ignores node_modules when bundling in Webpack. @see
		...
		// As of Webpack 2 beta, Webpack provides performance hints. Since we are not
		// targeting a browser, bundle size is not relevant. Additionally, the
		// performance hints clutter up our nice error messages.
		performance: {
			hints: false
		},
		...
		resolve: {
			...
			// modules: [config.userNodeModulesPath, path.resolve(__dirname,
			// '../node_modules')]
		},
		resolveLoader: {
			// modules: [config.userNodeModulesPath, path.resolve(__dirname,
		...
		// Define a few default Webpack loaders. Notice the use of the new Webpack 2
		// configuration: module.rules instead of module.loaders
		module: {
			rules: [
		...
		// A few commonly used plugins have been removed from Webpack v4. Now instead,
		// these plugins are avaliable as "optimizations". @see
		// https://webpack.js.org/configuration/optimization/
		optimization: {
			// optimization.noEmitOnErrors prevents Webpack The NoEmitOnErrorsPlugin plugin
			// prevents Webpack from printing out compile time stats to the console.
			noEmitOnErrors: true
		},
		plugins: [
			// We define some sensible Webpack flags. One for the Node environment, and one
			// for dev / production. These become global variables. Note if you use
			// something like eslint or standard in your editor, you will want to configure
			...
			// each file using the BannerPlugin.
			new webpack.BannerPlugin({
				raw: true, entryOnly: false, banner: `require('${
					// Is source-map-support installed as project dependency, or linked?require
			...
			// The FriendlyErrorsWebpackPlugin (when combined with source-maps) gives
